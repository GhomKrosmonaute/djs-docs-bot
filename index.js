...
const prefix = "*"
const types = ["class", "interface", "typedef", "prop", "method", "event"]
const support = "https://discord.gg/3vC2XWK"
let doc
let deprecated, github, npm, yay, labs, nul
let currentDocName = "stable"
...
client.on("ready", async function () {
  labs = await client.guilds.fetch("507389389098188820")
  deprecated = client.emojis.cache.get("605323302059376670")
  github = client.emojis.cache.get("605387724534317156")
  npm = client.emojis.cache.get("605387724102565890")
  yay = client.emojis.cache.get("557124850326437888")
  nul = client.emojis.cache.get("559100717445546087")
...
client.on("message", async function (message) {
  ...
  if (message.content === prefix + "versions") {
    const embed = new Discord.MessageEmbed()
      ...
      .setDescription(
        `Here are the ${data.libraries.length} Discord libraries supported by the bot as well as links to the various documentations available.\n${nul}`
      )
    ...
    const lib =
      data.libraries[Math.floor(Math.random() * data.libraries.length)]
    embed.setFooter(`Example : ${prefix}version ${lib.versions[0]}`, lib.image)
    await message.channel.send(embed)
  } else if (message.content === prefix + "info") {
    console.log(client.guilds.cache.map((guild) => guild.toString()).join("\n"))
    const embed = new Discord.MessageEmbed()
      .setColor(toLib(doc).color)
      .setAuthor(client.user.username, client.user.avatarURL({ dynamic: true }))
      .setDescription(
        `I help members of **${client.guilds.cache.size}** guilds.\n` +
    ...
    let embed = new Discord.MessageEmbed()
      ...
      .setThumbnail(
        "https://i1.wp.com/rubydiscord.bitnamiapp.com/wp-content/uploads/2017/12/discord-logo.png?resize=300%2C300"
      )
    ...
    embed = new Discord.MessageEmbed()
      ...
      .setImage(
        "https://cdn.discordapp.com/attachments/554206505897951252/605747204564451349/discorddevbanniere.png"
      )
    await message.channel
      .send(embed)
      .catch((error) => sendError(error, message.channel))
    ...
    if (message.content === "*") {
      const embed = embedDoc(doc).setAuthor(
        `Current documentation : ${toLib(doc).name}`,
        toLib(doc).image
      )
      message.channel
        .send(embed)
      ...
      const args = message.content.replace(prefix, "").trim().split(regex)
      let docElement = doc.get(...args)
      if (args.length === 0) return
      if (docElement === null) return
      message.channel
        .send(embedDoc(docElement))
        .catch((error) => sendError(error, message.channel))
    }
  }
})
...
function embedDoc(doc) {
  const embed = new Discord.MessageEmbed().setColor(toLib(doc).color)
  ...
  let title = `Structure : ${toLib(doc).repo}.${family
    .map((parent) => parent.docType)
    .join(".")}`
  ...
  if (doc.docType === "event") {
    author =
      author.replace(doc.name, "") +
      `on( "${doc.name}", callback( ${params
  ...
  if (doc.construct) {
    description.push(
      "```ts\n" +
        `class ${
          doc.construct.name
        }{\n\tconstructor(\n${doc.construct.params
          .map(
            (param) =>
              `\t\t${param.name}${param.optional ? "?" : ""}: ${param.type.join(
                " "
              )}`
          )
          .join(",\n")}\n\t)\n}` +
  ...
  types.forEach((type) => {
    ...
    if (children) {
      embed.addField(
        ...
        children
          .map((child) =>
            child.deprecated ? `${child.name} ${deprecated}` : child.name
          )
  ...
  if (doc.meta) {
    embed.setFooter(
      `→ ${doc.meta.path.replace(/\//g, " / ")} / ${doc.meta.file} - line ${
        doc.meta.line
      }`,
      toLib(doc).image
    )
...
function docpath(docElement) {
  return `${toLib(docElement).name} 🔹 ${
    docElement.project || docElement.doc.project
  }#${docElement.branch || docElement.doc.branch}`
}
